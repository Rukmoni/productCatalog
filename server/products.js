...
const add = (token, product) => {
  if (!product.id) {
    product.id = Math.random().toString(36).substr(-8)
  }

  get(token).products.push(product)
...
const remove = (token, id) => {
  ...
  const product = data.products.find(c => c.id === id)

  if (contact) {
    data.products = data.products.filter(c => c !== product)
  }
